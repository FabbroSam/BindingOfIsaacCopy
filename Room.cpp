...
void Room::Draw()
{
	...
	else if (_roomType == RoomType::TREASURE)
	{
		...
		Altar* altar = new Altar(_scene, RectF(_x + 7.5f, _y + 5.5f, 1, 1), spriteLoader->get("altar"));
		new Item(_scene, RectF(_x + 7.5f, _y + 4.8f, 1, 1), spriteLoader->get("item_poison"), 10, 0, SDL_FLIP_NONE, altar);
	}
