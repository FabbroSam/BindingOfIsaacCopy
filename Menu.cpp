...
MenuItem::MenuItem(Menu* container, int index, const std::string& text, std::function<void()> task)
	: RenderableObject(container,
		RectF(
			container->menuRect().left() + 1.2f + 0.1f * index,
		...
		//SpriteFactory::instance()->getText(' ' + text, { 0.5f, 0.5f }, 0, ' ', false))
{
	...
	_menuArrow = new RenderableObject(container,
		RectF(
			container->menuRect().left() + 0.2f + 0.1f * index,
			container->menuRect().top()  + 0.7f + 1.5f * index,
			1.4f, 2.2f),
...
Menu* Menu::mainMenu()
{
	Menu* menu = new Menu({ 4.2, 3.0 }, 5.5, 0, false);

	new RenderableObject(menu, RectF(-2.5, 0, 21, 15), SpriteFactory::instance()->get("menu_background"), -1);
	new RenderableObject(menu, RectF(-2.5, 0, 21, 15), SpriteFactory::instance()->get("menu_overlay"), -1);
	new RenderableObject(menu, RectF(-6, 5, 21, 15), SpriteFactory::instance()->get("menu_shading"), -1);

	menu->addItem("menu_new_run", []() 
	{
		Game::instance()->popSceneLater();
		Audio::instance()->playMusic("overworld");
	});
	menu->addItem("menu_options", [menu]()
	{
	...
	menu->addItem("menu_options", []() {Game::instance()->quit(); });
